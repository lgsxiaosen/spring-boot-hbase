...
public class TestMin {
    private final static Logger logger = LoggerFactory.getLogger(TestMin.class);
    ...
    static {
        ...
        configuration.set("hbase.zookeeper.quorum","10.42.121.132");  //hbase 服务地址
    ...
    public boolean creatTable(String tableName, List<String> columnFamily) {
        ...
        try {
            ...
            logger.error(MessageFormat.format("创建表{0}失败",tableName),e);
            return false;
        }finally {
    ...
    public boolean createTableBySplitKeys(String tableName, List<String> columnFamily, byte[][] splitKeys) {
        ...
        try {
            if (StringUtils.isBlank(tableName) || columnFamily == null
                    || columnFamily.size() == 0) {
                logger.error("===Parameters tableName|columnFamily should not be null,Please check!===");
                return false;
            }
            admin = connection.getAdmin();
            if (admin.tableExists(TableName.valueOf(tableName))) {
...
//                System.out.println("-------------------------------------------");
            ...
            logger.error(MessageFormat.format("查询一行的数据失败,tableName:{0},rowKey:{1}"
                    ,tableName,rowKey),e);
        }finally {
    ...
    public List<String> getColumnValuesByVersion(String tableName, String rowKey, String familyName, String columnName,int versions) {
        ...
        try {
            ...
            logger.error(MessageFormat.format("查询指定单元格多个版本的数据失败,tableName:{0},rowKey:{1},familyName:{2},columnName:{3}"
                    ,tableName,rowKey,familyName,columnName),e);
        }finally {
            close(null,null,table);
        }
    ...
    public void putData(String tableName,String rowKey, String familyName, String[] columns, String[] values) {
        ...
        try {
            ...
            logger.error(MessageFormat.format("为表添加 or 更新数据失败,tableName:{0},rowKey:{1},familyName:{2}"
    ...
    public void setColumnValue(String tableName, String rowKey, String familyName, String column1, String value1){
        ...
        try {
            ...
            logger.error(MessageFormat.format("为表的某个单元格赋值失败,tableName:{0},rowKey:{1},familyName:{2},column:{3}"
    ...
    public boolean deleteColumn(String tableName, String rowKey, String familyName, String columnName) {
        ...
        try {
            ...
            if(admin.tableExists(TableName.valueOf(tableName))){
                ...
                table.delete(delete);
                logger.debug(MessageFormat.format("familyName({0}):columnName({1})is deleted!",familyName,columnName));
            }

        }catch (IOException e) {
            logger.error(MessageFormat.format("删除指定的列失败,tableName:{0},rowKey:{1},familyName:{2},column:{3}"
                    ,tableName,rowKey,familyName,columnName),e);
            return false;
        }finally {
    ...
    public boolean deleteRow(String tableName, String rowKey) {
        ...
        try {
            ...
            if(admin.tableExists(TableName.valueOf(tableName))){
                ...
                table.delete(delete);
                logger.debug(MessageFormat.format("row({0}) is deleted!",rowKey));
            }
        }catch (IOException e) {
            logger.error(MessageFormat.format("删除指定的行失败,tableName:{0},rowKey:{1}"
                    ,tableName,rowKey),e);
            return false;
        }finally {
            close(admin,null,table);
    ...
    public boolean deleteColumnFamily(String tableName, String columnFamily) {
        ...
        try {
            ...
            if(admin.tableExists(TableName.valueOf(tableName))){
                ...
                logger.debug(MessageFormat.format("familyName({0}) is deleted!",columnFamily));
            }
        }catch (IOException e) {
            logger.error(MessageFormat.format("删除指定的列族失败,tableName:{0},columnFamily:{1}"
    ...
    public boolean deleteTable(String tableName){
        ...
        try {
            ...
            logger.error(MessageFormat.format("删除指定的表失败,tableName:{0}"
                    ,tableName),e);
            return false;
        }finally {
            close(admin,null,null);
    ...
    public static void main(String[] args) throws IOException{
        ...
        Map<String,String> mapMap = testMin.getRowData("avatar_human_feature", "19647368");
        logger.info("根据tableName和rowKey精确查询一行的数据:{}", mapMap);
